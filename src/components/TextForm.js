  ...
  return (
   ...
   <div className='container' style={{color:props.mode==='dark'?'white':'black'}}>
    ...
    <div className="mb-3">
      ...
      } style={{backgroundColor:props.mode==='dark'?'#171616':'white',color:props.mode==='dark'?'white':'black'}} onChange={handleOnChange}></textarea>
      ...
      <button disabled={text.length===0} className="btn btn-primary mx-1 my-3" onClick={handleInverseCaseClick}>InverseCase Text</button>

      <button disabled={text.length===0} className="btn btn-primary mx-1 my-3" onClick={handleClearClick}>Clear Text</button>

      <button disabled={text.length===0} className="btn btn-primary mx-1 my-3" onClick={handleCopyClick}>Copy Text</button>

      <button disabled={text.length===0} className="btn btn-primary mx-1 my-3" onClick={handleExtraSpacesClick}>Remove Extra Spaces</button>
    ...
    <div className="container my-3" style={{color:props.mode==='dark'?'white':'#042743'}}>
      ...
      {/* /[ ]+/ */}
      <p><b>{text?text.trim().split(/\s+/).length:0}</b> words and <b>{text.length} </b>characters</p>
